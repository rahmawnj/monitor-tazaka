<style>
    ...
    @media (prefers-reduced-motion: reduce) { .project.water-update, .project.water-update::before, .detail-bar span.realtime-update { animation: none !important; } }
</style>
...
(() => {
    ...
    const animateProgressBar = (projectId, oldProgress, newProgress) => {
        ...
        const from = Math.max(0, Math.min(100, Number(oldProgress) || 0));
        const to = Math.max(0, Math.min(100, Number(newProgress) || 0));
        bar.style.transition = 'none'; bar.style.width = `${from}%`; bar.style.background = progressGradient(to);
        if (label) label.textContent = `${from}%`;
        void bar.offsetWidth;
        requestAnimationFrame(() => {
            bar.style.transition = 'width 1.5s cubic-bezier(.16,1,.3,1), box-shadow .8s ease';
            bar.style.boxShadow = '0 0 16px rgba(255,255,255,.65)'; bar.style.width = `${to}%`;
            if (label) {
                const start = performance.now();
                const tick = now => { const p=Math.min(1,(now-start)/1500); const eased=1-Math.pow(1-p,3); label.textContent=`${Math.round(from+(to-from)*eased)}%`; if(p<1)requestAnimationFrame(tick);else label.textContent=`${to}%`; };
                requestAnimationFrame(tick);
            }
        });
    ...
    const refreshMonitor = async (event = {}) => {
        try {
            ...
            if (changedProject && progressChanged) requestAnimationFrame(() => {
    ...
    const pusher = new Pusher(@json(env('REVERB_APP_KEY')), {
        cluster:'mt1', wsHost:@json(env('REVERB_HOST','127.0.0.1')), wsPort:Number(@json(env('REVERB_PORT',8085))),
        wssPort:Number(@json(env('REVERB_PORT',8085))), forceTLS:@json(env('REVERB_SCHEME','http'))==='https', enabledTransports:['ws','wss'], disableStats:true
    });
