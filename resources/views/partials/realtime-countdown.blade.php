...
(function () {
    ...
    function findProjectById(id) {
        if (!Array.isArray(window.initialProjects)) return null;
        return window.initialProjects.find(p => String(p.id ?? '') === String(id)) ?? null;
    }

    function updateCountdowns() {
        const projects = Array.isArray(window.initialProjects) ? window.initialProjects : [];

        document.querySelectorAll('.project[data-project-id]').forEach(card => {
            const project = findProjectById(card.dataset.projectId);
            if (!project || !project.target_completion_date) return;
            ...
            if (!el) {
                ...
                el.className = 'project-countdown';
